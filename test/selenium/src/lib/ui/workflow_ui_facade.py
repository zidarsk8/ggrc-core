...
def create_task_group_task(task_group_task):
  ...
  if not task_group_task.start_date:
    task_group_task.start_date = datetime.date.today()
  if not task_group_task.due_date:
    task_group_task.due_date = task_group_task.start_date + datetime.timedelta(
        days=7)
