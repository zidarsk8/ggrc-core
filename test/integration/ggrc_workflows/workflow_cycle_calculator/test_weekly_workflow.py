...
class TestWeeklyWorkflow(base_workflow_test_case.BaseWorkflowTestCase):
  """Test cases for weekly workflows"""
  ...
  def test_relative_to_day(self):
    ...
    rtd = wcc.WeeklyCycleCalculator(active_wf).relative_day_to_date
    ...
    with freezegun.freeze_time("2015-6-8 13:00:00"):  # Monday, 6/8/2015
      self.assertEqual(rtd("1"), datetime.date(2015, 6, 8))
      self.assertEqual(rtd("2"), datetime.date(2015, 6, 9))
      self.assertEqual(rtd("3"), datetime.date(2015, 6, 10))
      self.assertEqual(rtd("4"), datetime.date(2015, 6, 11))
      self.assertEqual(rtd("5"), datetime.date(2015, 6, 12))
  ...
  def test_start_workflow_mid_cycle_with_task_before_and_after(self):
    ...
    with freezegun.freeze_time("2015-07-29 13:00"):  # Wed
      _, wf = self.generator.generate_workflow(weekly_wf)
      _, awf = self.generator.activate_workflow(wf)
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id).one()
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 27))
      self.assertEqual(cycle.end_date, datetime.date(2015, 7, 31))

      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 8, 3))
  ...
  def test_workflow_created_on_weekend_is_scheduled_activate_next_week(self):
    ...
    with freezegun.freeze_time("2015-07-25 13:00"):  # Sat
      _, wf = self.generator.generate_workflow(weekly_wf)
      _, awf = self.generator.activate_workflow(wf)
  ...
  def test_delete_all_t_after_cs_were_already_created_and_create_new_tg(self):
    ...
    with freezegun.freeze_time("2015-6-9 13:00:00"):  # Tuesday, 6/9/2015
      ...
      self.assertEqual(cycle.end_date, datetime.date(2015, 6, 12))
      ...
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 6, 17))
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 6, 24))
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 1))
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 8))
      ...
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 14))
  ...
  def test_cycle_start_date_after_task_start_date_moves_backward(self):
    """Test that modifying tasks after cycles exists to lesser values (from
    friday to monday) correctly calculates for next cycle start date."""
    weekly_wf = {
        "title": "weekly thingy",
        "description": "start this many a time",
        "frequency": "weekly",
    ...
    with freezegun.freeze_time("2015-7-30 13:00:00"):  # Thu, 7/30/2015
      _, wf = self.generator.generate_workflow(weekly_wf)
      _, awf = self.generator.activate_workflow(wf)
      ...
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 31))
      self.assertEqual(cycle.end_date, datetime.date(2015, 8, 6))
      ...
      self.assertEqual(cycle.start_date, datetime.date(2015, 8, 31))
      self.assertEqual(cycle.end_date, datetime.date(2015, 9, 3))
      ...
      self.assertEqual(cycle.start_date, datetime.date(2015, 9, 14))
      self.assertEqual(cycle.end_date, datetime.date(2015, 9, 17))

  def test_create_c_manually_start_delete_and_create_t_on_the_same_day(self):
    """Test if NCSD get's calculated correctly if we create a task,
    manually start the cycle, delete task and recreate
    task with the same start day"""
    weekly_wf = {
        "title": "weekly thingy",
        "description": "start this many a time",
        "frequency": "weekly",
    ...
    with freezegun.freeze_time("2015-08-10 05:00"):
      _, wf = self.generator.generate_workflow(weekly_wf)
      _, awf = self.generator.activate_workflow(wf)
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id).one()
      self.assertEqual(cycle.start_date, datetime.date(2015, 8, 10))
      self.assertEqual(cycle.end_date, datetime.date(2015, 8, 10))

      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 8, 17))
      self.assertEqual(cycle.end_date, datetime.date(2015, 8, 17))
      ...
      task_group = db.session.query(models.TaskGroup).filter(
          models.TaskGroup.workflow_id == wf.id).one()
      old_task = db.session.query(models.TaskGroupTask).filter(
          models.TaskGroupTask.task_group_id == task_group.id).one()
      ...
      _, tgt = self.generator.generate_task_group_task(
          task_group, data=new_task)
