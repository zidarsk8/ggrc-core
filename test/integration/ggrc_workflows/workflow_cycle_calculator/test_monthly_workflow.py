...
class TestMonthlyWorkflow(base_workflow_test_case.BaseWorkflowTestCase):
  ...
  def test_monthly_workflow(self):
    ...
    with freeze_time("2015-7-14 13:00:00"):  # Tuesday, 7/14/2015
      ...
      self.assertEqual(active_wf.next_cycle_start_date,
                       date(2015, 8, 14))  # 8/14/2015 Friday
  ...
  def test_delete_all_tasks(self):
    """Test delet all tasks.

    Check that workflow doesn't have next cycle start date when all tasks are
    deleted.
    """
    monthly_workflow = {
        "title": "monthly test wf",
  ...
  def test_type_casting(self):
    ...
    with freeze_time("2015-7-15 13:00"):  # Wed
      ...
      self.assertEqual(active_wf.next_cycle_start_date,
                       date(2015, 8, 14))  # 15 is Sat

  def test_ncsd_in_previous_month_automatic(self):
    """Check start dates in previos month.

    Check starting cycles on periodic basis with one of next cycle start dates
    in previous month
    """
    monthly_workflow = {
        "title": "monthly test wf",
  ...
  def test_manual_starting_cycles_ncsd_in_previous_month(self):
    """Check starting cycles manually with one next cycle start date in the
    previous month.
    """
    monthly_workflow = {
        "title": "monthly test wf",
  ...
  def test_delete_task_after_cycles(self):
  ...
  def test_delete_tasks_before_ncsd(self):
    """Check that workflow doesn't reset next cycle start date when all tasks
    are deleted after cycles were already created.
    """
    monthly_workflow = {
  ...
  def test_setup_tasks_didnt_get_deleted_cycle_tasks_got_deleted(self):
    """Test usecase where user deletes latest cycle's tasks but NOT setup
    tasks."""
    monthly_workflow = {
        "title": "monthly test wf",
        "description": "start this many a time",
