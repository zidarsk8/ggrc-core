...
# pylint: disable=invalid-name,unused-variable,too-many-lines


class TestMonthlyWorkflow(base_workflow_test_case.BaseWorkflowTestCase):
  """Test case for monthly workflows"""
  ...
  def test_monthly_workflow(self):
    ...
    with freezegun.freeze_time("2015-6-9 13:00:00"):  # Tuesday, 6/9/2015
      ...
      self.assertEqual(cycle.start_date, datetime.date(2015, 6, 12))
    ...
    with freezegun.freeze_time("2015-7-14 13:00:00"):  # Tuesday, 7/14/2015
      ...
      cycle = db.session.query(models.Cycle).filter(
          ...
          models.Cycle.start_date == datetime.date(2015, 7, 14)).one()

      self.assertEqual(active_wf.next_cycle_start_date,
                       datetime.date(2015, 8, 14))  # 8/14/2015 Friday
      self.assertEqual(cycle.start_date, datetime.date(
          2015, 7, 14))  # 7/10/2015 Fri
      self.assertEqual(cycle.end_date, datetime.date(
          2015, 8, 3))  # 8/3/2015 Mon
  ...
  def test_change_task_dates(self):
    ...
    with freezegun.freeze_time("2015-7-1 14:00:00"):  # 7/1/2015 Wed
      _, wf = self.generator.generate_workflow(monthly_workflow)
      _, awf = self.generator.activate_workflow(wf)
      ...
      task = db.session.query(models.TaskGroupTask).filter(
          models.TaskGroupTask.task_group_id == task_group.id).one()
  ...
  def test_update_next_cycle_start_date_delete_task(self):
    ...
    with freezegun.freeze_time("2015-6-9 13:00:00"):  # Tuesday, 6/9/2015
      ...
      task_group = db.session.query(models.TaskGroup).filter(
          models.TaskGroup.workflow_id == wf.id).one()
      task = db.session.query(models.TaskGroupTask).filter(
          models.TaskGroupTask.task_group_id == task_group.id,
          models.TaskGroupTask.relative_start_day == 14
      ).one()
  ...
  def test_update_next_cycle_start_date_delete_task_group(self):
    ...
    with freezegun.freeze_time("2015-6-9 13:00:00"):  # Tuesday, 6/9/2015
      ...
      task_group = db.session.query(models.TaskGroup).filter(
          models.TaskGroup.workflow_id == wf.id,
          models.TaskGroup.title == u"task group 2"
      ).one()
  ...
  def test_update_next_cycle_start_date_new_task_group(self):
    ...
    with freezegun.freeze_time("2015-6-9 13:00:00"):  # Tuesday, 6/9/2015
      ...
      _, tg = self.generator.generate_task_group(wf, data=data)
  ...
  def test_delete_all_tasks(self):
    """Check that workflow doesn"t have next cycle start date when
  ...
  def test_type_casting(self):
    ...
    with freezegun.freeze_time("2015-7-1 13:00"):  # Wed
      _, wf = self.generator.generate_workflow(monthly_workflow)
    ...
    with freezegun.freeze_time("2015-7-15 13:00"):  # Wed
      start_recurring_cycles()
      ...
      self.assertEqual(cycle.end_date, datetime.date(2015, 7, 17))  # 19 is Sun
      self.assertEqual(active_wf.next_cycle_start_date,
                       datetime.date(2015, 8, 14))  # 15 is Sat

  def test_ncsd_in_previous_month_automatic(self):
    """Check starting cycles on periodic basis with one of
    next cycle start dates in previous month"""
    monthly_workflow = {
        "title": "monthly test wf",
        "description": "start this many a time",
        "frequency": "monthly",
    ...
    with freezegun.freeze_time("2015-5-30 14:00"):
      _, wf = self.generator.generate_workflow(monthly_workflow)
      _, awf = self.generator.activate_workflow(wf)
    ...
    with freezegun.freeze_time("2015-6-1 01:00"):
      start_recurring_cycles()
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id,
          models.Cycle.start_date == datetime.date(2015, 6, 1)).one()

      self.assertEqual(cycle.start_date, datetime.date(2015, 6, 1))
      self.assertEqual(cycle.end_date, datetime.date(2015, 6, 15))

    # Jul

    with freezegun.freeze_time("2015-7-1 01:00"):
      start_recurring_cycles()
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id,
          models.Cycle.start_date == datetime.date(2015, 7, 1)).one()

      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 1))
      self.assertEqual(cycle.end_date, datetime.date(2015, 7, 15))

    # Aug

    with freezegun.freeze_time("2015-7-31 01:00"):
      start_recurring_cycles()
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id,
          models.Cycle.start_date == datetime.date(2015, 7, 31)).one()

      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 31))
      self.assertEqual(cycle.end_date, datetime.date(2015, 8, 14))

    # Sept

    with freezegun.freeze_time("2015-9-1 01:00"):
      start_recurring_cycles()
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id,
          models.Cycle.start_date == datetime.date(2015, 9, 1)).one()

      self.assertEqual(cycle.start_date, datetime.date(2015, 9, 1))
      self.assertEqual(cycle.end_date, datetime.date(2015, 9, 15))

  def test_manual_starting_cycles_ncsd_in_previous_month(self):
    """Check starting cycles manually with one next cycle start date in
    the previous month"""
    monthly_workflow = {
        "title": "monthly test wf",
        "description": "start this many a time",
    ...
    with freezegun.freeze_time("2015-7-1 13:00:00"):  # 7/1/2015 Wed
      ...
      # models.Workflow should have one active cycle and
      ...
      # Oct
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 10, 1))
      self.assertEqual(cycle.end_date, datetime.date(2015, 10, 15))
      ...
      # Nov
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 10, 30))
      self.assertEqual(cycle.end_date, datetime.date(2015, 11, 13))
      ...
      # Dec
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 12, 1))
      self.assertEqual(cycle.end_date, datetime.date(2015, 12, 15))
      ...
      # Jan
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 12, 30))
      self.assertEqual(cycle.end_date, datetime.date(2016, 1, 15))
  ...
  def test_start_date_late_in_month(self):
    ...
    with freezegun.freeze_time("2015-1-20 13:00"):
      _, wf = self.generator.generate_workflow(monthly_workflow)
      _, awf = self.generator.activate_workflow(wf)
    ...
    # January
    with freezegun.freeze_time("2015-1-29 14:00"):
      start_recurring_cycles()
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id,
          models.Cycle.start_date == datetime.date(2015, 1, 29)).one()

      self.assertEqual(cycle.start_date, datetime.date(2015, 1, 29))
      self.assertEqual(cycle.end_date, datetime.date(2015, 1, 30))

    # February
    with freezegun.freeze_time("2015-2-27 14:00"):
      start_recurring_cycles()
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id,
          models.Cycle.start_date == datetime.date(2015, 2, 27)).one()

      self.assertEqual(cycle.start_date, datetime.date(2015, 2, 27))
      self.assertEqual(cycle.end_date, datetime.date(2015, 2, 27))

    # March
      with freezegun.freeze_time("2015-3-27 14:00"):
        start_recurring_cycles()
        ...
        cycle = db.session.query(models.Cycle).filter(
            models.Cycle.workflow_id == wf.id,
            models.Cycle.start_date == datetime.date(2015, 3, 27)).one()

        self.assertEqual(cycle.start_date, datetime.date(2015, 3, 27))
        self.assertEqual(cycle.end_date, datetime.date(2015, 3, 31))

    # April
      with freezegun.freeze_time("2015-4-29 14:00"):
        start_recurring_cycles()
        ...
        cycle = db.session.query(models.Cycle).filter(
            models.Cycle.workflow_id == wf.id,
            models.Cycle.start_date == datetime.date(2015, 4, 29)).one()

        self.assertEqual(cycle.start_date, datetime.date(2015, 4, 29))
        self.assertEqual(cycle.end_date, datetime.date(2015, 4, 30))
  ...
  def test_changing_end_date_when_tasks_deleted(self):
    ...
    with freezegun.freeze_time("2015-07-15 13:00"):
      _, wf = self.generator.generate_workflow(monthly_workflow)
      _, awf = self.generator.activate_workflow(wf)
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id,
          models.Cycle.start_date == datetime.date(2015, 7, 15)).one()
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 15))
      self.assertEqual(cycle.end_date, datetime.date(2015, 7, 24))

      cycle_task = db.session.query(models.CycleTaskGroupObjectTask).filter(
          models.CycleTaskGroupObjectTask.cycle_id == cycle.id,
          models.CycleTaskGroupObjectTask.title == "monthly task 2"
      ).one()
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id).one()
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 15))
      self.assertEqual(cycle.end_date, datetime.date(2015, 7, 20))
  ...
  def test_delete_task_after_workflow_has_been_activated(self):
    ...
    with freezegun.freeze_time("2015-07-22 13:00"):
      _, wf = self.generator.generate_workflow(monthly_workflow)
      _, awf = self.generator.activate_workflow(wf)
      ...
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 24))
      self.assertEqual(cycle.end_date, datetime.date(2015, 7, 29))
      ...
      task = db.session.query(models.TaskGroupTask).filter(
          models.TaskGroupTask.task_group_id == task_group.id,
          models.TaskGroupTask.title == "monthly task 1"
      ).one()
  ...
  def test_start_workflow_mid_cycle_with_task_before_and_after(self):
    """Test that workflows get triggered correctly if we are in the middle
    of the cycle and there are tasks with start dates before and after."""
    monthly_workflow = {
        "title": "monthly test wf",
        "description": "start this many a time",
        "frequency": "monthly",
    ...
    with freezegun.freeze_time("2015-08-03 13:00"):
      _, wf = self.generator.generate_workflow(monthly_workflow)
      _, awf = self.generator.activate_workflow(wf)
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id).one()
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 31))
      self.assertEqual(cycle.end_date, datetime.date(2015, 8, 5))
  ...
  def test_delete_all_ts_after_cs_were_already_created_and_create_new_tg(self):
    """Check that workflow doesn"t reset next cycle start date when all tasks
    are deleted after cycles were already created"""
    monthly_workflow = {
        "title": "monthly test wf",
        "description": "start this many a time",
        "frequency": "monthly",
    ...
    with freezegun.freeze_time("2015-6-9 13:00:00"):  # Tuesday, 6/9/2015
      ...
      self.assertEqual(cycle.end_date, datetime.date(2015, 6, 19))
      ...
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 9, 11))
      self.assertEqual(cycle.end_date, datetime.date(2015, 10, 7))
  ...
  def test_delete_all_ts_after_cs_were_already_created_no_new_ts_created_until_NCSD(self):  # noqa
    """Check that workflow doesn"t reset next cycle start date when all
    tasks are deleted after cycles were already created"""
    monthly_workflow = {
        "title": "monthly test wf",
        "description": "start this many a time",
        "frequency": "monthly",
    ...
    with freezegun.freeze_time("2015-9-15 13:00"):
      start_recurring_cycles()
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.workflow_id == wf.id,
          models.Cycle.start_date == datetime.date(2015, 9, 15)
      )
  ...
  def test_cycle_task_delition(self):
    ...
    with freezegun.freeze_time("2015-6-9 13:00:00"):
      ...
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 6, 15))
      self.assertEqual(cycle.end_date, datetime.date(2015, 6, 23))

      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 7, 15))
      self.assertEqual(cycle.end_date, datetime.date(2015, 7, 23))

      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 8, 14))
      self.assertEqual(cycle.end_date, datetime.date(2015, 8, 21))

      cycle_task_1 = db.session.query(models.CycleTaskGroupObjectTask).filter(
          models.CycleTaskGroupObjectTask.cycle_id == cycle.id,
          models.CycleTaskGroupObjectTask.start_date == datetime.date(
              2015, 8, 14),
          models.CycleTaskGroupObjectTask.title == "monthly task 1"
      ).one()

      cycle_task_2 = db.session.query(models.CycleTaskGroupObjectTask).filter(
          models.CycleTaskGroupObjectTask.cycle_id == cycle.id,
          models.CycleTaskGroupObjectTask.start_date == datetime.date(
              2015, 8, 17),
          models.CycleTaskGroupObjectTask.title == "monthly task 2"
      ...
      _, cycle = self.generator.generate_cycle(wf)
      self.assertEqual(cycle.start_date, datetime.date(2015, 9, 15))
      self.assertEqual(cycle.end_date, datetime.date(2015, 9, 23))
  ...
  def test_setup_tasks_deleted_last_cycle_tasks_deleted(self):
    ...
    with freezegun.freeze_time("2015-6-9 13:00:00"):
      ...
      cycle = db.session.query(models.Cycle).filter(
          models.Cycle.id == cycle.id).one()
      self.assertEqual(cycle.start_date, datetime.date(2015, 8, 17))
