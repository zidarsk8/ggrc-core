...
"""Test Access Control roles Secondary Assignees propagation"""
...
@ddt.ddt
class TestSecondaryAssigneesPropagation(base.TestACLPropagation):
  """Test Secondary Assignees role permissions propagation
  ...
  Control's Secondary Assignees can read/create etc document and its comments.
  """
  ...
  def init_factory(self, role, model, parent):
    """Initialize RBAC factory with propagated Secondary Assignees role.
  ...
  def test_access(self, role, model, action_name, expected_result):
    """Secondary Assignees {0:<7}: On {1:<20} test {2:<20} - Expected {3:<2} """
