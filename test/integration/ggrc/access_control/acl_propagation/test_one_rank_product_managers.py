...
"""Test Access Control roles Product Managers propagation"""
...
@ddt.ddt
class TestProductManagersPropagation(base.TestACLPropagation):
  """Test Product Managers role permissions propagation
  ...
  def init_factory(self, role, model, parent):
    """Initialize RBAC factory with propagated Product Managers role.
    ...
    """
    self.setup_people()
    product_managers = all_models.AccessControlRole.query.filter_by(
        name="Product Managers",
        object_type=parent,
    ).first()
    rbac_factory = rbac_factories.TEST_FACTORIES_MAPPING[model]
    return rbac_factory(self.people[role].id, product_managers, parent)

  @helpers.unwrap(PERMISSIONS)
  def test_access(self, role, model, action_name, expected_result):
    """Product Managers {0:<7}: On {1:<20} test {2:<20} - Expected {3:<2} """
