...
"""Test Access Control roles System Owners propagation"""
...
@ddt.ddt
class TestSystemOwnersPropagation(base.TestACLPropagation):
  """Test System Owners role permissions propagation
  ...
  def init_factory(self, role, model, parent):
    """Initialize RBAC factory with propagated System Owners role.
    ...
    """
    self.setup_people()
    system_owners = all_models.AccessControlRole.query.filter_by(
        name="System Owners",
        object_type=parent,
    ).first()
    rbac_factory = rbac_factories.TEST_FACTORIES_MAPPING[model]
    return rbac_factory(self.people[role].id, system_owners, parent)

  @helpers.unwrap(PERMISSIONS)
  def test_access(self, role, model, action_name, expected_result):
    """System Owners {0:<7}: On {1:<20} test {2:<20} - Expected {3:<2} """
