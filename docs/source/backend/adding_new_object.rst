...
In :src:`ggrc-client/js/templates/dashboard/lhn.mustache` add the line:

..  code-block:: javascript

    {{{renderLive '/static/templates/dashboard/lhn_search.mustache' type="NewObject" li_class="class"}}}

Create mustache files
---------------------

``new_object/modal_content.mustache`` is the only mandatory template
