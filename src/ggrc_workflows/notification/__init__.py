...
def get_workflow_for_task(task_id):
  ...
  if workflow_task is not None:
    return db.session.filter(models.Workflow.id == workflow_task.workflow_id).first()
  else:
   return None

def get_cycle_for_task(taskgroup_object_id):
  taskgroup= db.session.query(models.CycleTaskGroup).\
    join(models.CycleTaskGroupObjects).\
    filter(models.TaskGroupObjects.id == taskgroup_object_id)
  if taskgroup is not None:
    return db.session.filter(models.Cycle.id == taskgroup.cycle_id).first()
  else:
   return None

def handle_tasks_overdue():
  tasks = db.session.filter(models.CycleTaskGroupObjectTask.end_date > datetime.today()).all()
  for task in tasks:
    ...
    prepare_notification(task, notif_type, notif_pri, subject, content, task.owner, recipients, False)
   db.session.commit()

def handle_task_due(num_days):
  tasks = db.session.filter(models.CycleTaskGroupObjectTask.end_date > datetime.today()-timedelta(num_days)).all()
  for task in tasks:
    ...
    recipients = [task.contact]
    prepare_notification(task, notif_type, notif_pri, subject, content, task.contact, recipients, False)
   db.session.commit()
  ...
  for cycle in workflow_cycles:
    ...
    for task in workflow_obj.tasks:
      if task.status != status:
        ...
        recipients = [task.owner]
        prepare_notification(task, notif_type, notif_pri, subject, content, cycle.owner, recipients, False)
  db.session.commit()
...
def handle_task_put(sender, obj=None, src=None, service=None):
  ...
  if obj.status in ['Declined', 'Verified']: 
    ...
    recipients = [cyle_obj.contact]
    prepare_notification(src, notif_type, notif_pri, subject, content, src.contact, recipients, override)
