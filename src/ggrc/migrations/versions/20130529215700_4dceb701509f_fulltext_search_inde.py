...
def upgrade():
  ...
  if db_scheme == 'mysql':
    ...
    ddl = """ALTER TABLE {tablename}
             ADD FULLTEXT INDEX {tablename}_text_idx (content)"""
    op.execute(ddl.format(tablename='fulltext_record_properties'))
  elif db_scheme == 'sqlite':
    ddl = """CREATE VIRTUAL TABLE {tablename}
             USING fts4(key, type, tags, property, content)"""
    op.execute(ddl.format(tablename='fulltext_record_properties'))
  else:
    print("Unsupported db scheme {db_scheme} -- should be one of mysql, "
          "sqlite".format(db_scheme=db_scheme))
