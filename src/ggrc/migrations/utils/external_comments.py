...
def _get_comments_ids_by_obj_type(connection, obj_type, is_external=False):
  ...
  result = connection.execute(
    sa.text("""
      SELECT source_id
      FROM relationships
      WHERE source_type = :comment_type AND destination_type = :obj_type

      UNION

      SELECT destination_id
      FROM relationships
      WHERE source_type = :obj_type AND destination_type = :comment_type
    """),
  ...
  if old_rels:
    ...
    new_rels = connection.execute(
      sa.text("""
        SELECT id
        FROM relationships
        WHERE (source_type = 'ExternalComment' AND destination_type = :obj_type) OR
              (destination_type = 'ExternalComment' AND source_type = :obj_type) 
        """),
      obj_type=obj_type).fetchall()
